...
```{r}
getwd()
setwd("E:/individual_leaf/inexperienced raters")
```
...
```{r message=FALSE, warning=FALSE}
dat_cls <- read.csv("E:/individual_leaf/inexperienced raters/inexperienced_raters.csv")
attach(dat_cls)
```
...
dat_sad %>%
  ...
  geom_vline(aes(xintercept = actual), linetype = 2, color = "red", size = 0.5) +
  theme_light()+
  geom_rug(aes(actual))+
  geom_histogram(bins = 100, color = "white", fill = "grey") +
...
Let's now check the distribution of the unaided estimates for each of the aid systems.
...
knots_un <- dat_sad %>%
 ...
 labs(x = "Unaided Estimate", y = "Frequency (%)")+theme(axis.text.x = element_text(angle = 90,size = 15))+
...
fig3a
ggsave("fig_3a,Absolute error for unexperienced raters.JPG",  height=8,width=7,dpi = 300)
```
...
s1=dat_sad %>%
  ggplot(aes(actual, error)) +
  geom_point(alpha = 0.3, linewidth = 2) +
  geom_smooth(color = "orange", linewidth = 2, linetype = 1, se = F) +
  geom_hline(yintercept = 0) +
   theme_light()+
  theme(legend.position = "none") +
  facet_wrap(~ rater*assessment, ncol = 6,scales = "fixed") +
...
ggsave("unaided inexperienced raters.JPG",  height=8,width=7,dpi = 300)
```
...
s1=dat_sad %>%
  ...
  theme(legend.position = "none") +
  facet_wrap(~ rater*assessment, ncol = 6,scales = "fixed") +
  labs(y = "Estimate severity (%)", x = "Actual severity (%)")+
  theme(axis.text.x = element_text(angle = 90,size = 12))+
  theme(axis.text.y = element_text(angle = 0,size = 12))+
  theme(axis.title.x = element_text(size = 12))+
  theme(axis.title.y = element_text(size = 12))
...
```{r}
knots_aid <- dat_sad%>%
  ggplot(aes(estimate)) +facet_wrap(~rater)+
  geom_vline(aes(xintercept = actual), linetype = 2, color = "red", size = 0.5) +
  geom_rug(aes(actual))+
  geom_histogram(bins = 100, color = "white", fill = "gray50") +
   theme_light()+
   facet_wrap(~ assessment, ncol = 1) +
 labs(x = "Disease score", y = "Frequency (%)")+theme(axis.text.x = element_text(angle = 90,size = 15))+
  theme(axis.text.y = element_text(angle = 0,size = 15))+
  theme(axis.title.x = element_text(size = 15))+
  theme(axis.title.y = element_text(size = 15))+
  theme(axis.title.x = element_text(size = 15))+
...
knots_aid <- dat_sad %>%
  ...
  geom_histogram(bins = 100, color = "white", fill = "gray50") +
   theme_light()+
   facet_wrap(~ assessment, ncol = 1) +
 labs(x = "Aided Estimate", y = "Frequency (%)")+theme(axis.text.x = element_text(angle = 90,size = 15))+
  theme(axis.text.y = element_text(angle = 0,size = 15))+
  theme(axis.title.x = element_text(size = 15))+
  theme(axis.title.y = element_text(size = 15))+
  theme(axis.title.x = element_text(size = 15))+
...
knots_aid
```
combining the raters

```{r}
library(ggplot2)
library(gridExtra)
library(cowplot)
estimate=plot_grid(knots_aid,knots_un)
estimate

ggsave("estimate.JPG",  height=8,width=7,dpi = 300)
```
...
labs(y = "Absolute error", x = "Actual severity (%)") 
fig3b
ggsave("fig_3b.JPG",  height=8,width=7,dpi = 300)
```
...
s2=dat_sad %>%
  ...
  theme(axis.title.x = element_text(size = 17))+
  theme(axis.title.y = element_text(size = 17))+
  facet_wrap(.~ assessment, ncol = 6) +
  labs(y = "Absolute error (%)", x = "Actual severity (%)")
ggsave("s2 All rater errors.JPG",  height=8,width=7,dpi = 300)
s2
...
ggsave("fig_all.JPG",  height=8,width=7,dpi = 300)
```
...
p_lin_table <- lin_table %>%
  gather(AID, value, 2:11) %>%
  mutate(AID = as.numeric(AID)) %>%
  ggplot(aes(actual_LIN, AID, fill = value)) +
  geom_tile() +
...
ccc_all
```
extracting the standard deviation values
```{r}
sd(ccc_UN_df$s.shift)
sd(ccc_UN_df$l.shift)
sd(ccc_UN_df$Cb)
sd(ccc_UN_df$est)
sd(ccc_UN_df$r)
```
```{r}
sd(ccc_estimate_LIN_df$s.shift)
sd(ccc_estimate_LIN_df$l.shift)
sd(ccc_estimate_LIN_df$Cb)
sd(ccc_estimate_LIN_df$est)
sd(ccc_estimate_LIN_df$r)
```
...
```{r}
write.csv(ccc_all,"correlation coefficients for inexperienced raters.csv")
```
...
ccc5$rater=as.factor(ccc5$rater)
ccc5$method=as.factor(ccc5$method)
```
...
fig5c=ccc5 %>%
  ...
  geom_boxplot() +
   theme_light()+
  facet_wrap(~ baseline_class) +
  labs(x = "Rating system", y = "Pearson's coefficient") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
fig5c
```
...
```{r}
write.csv(table12,"summary of ratings for unexperienced raters.csv")
```
...
ccc6 <- ccc5 %>%
  filter(method != "UN")
m1=table12 %>% filter(method=="UN")
m2=table12 %>% filter(method!="UN")
```
...
sad_icc_UN <- irr::icc(sad_UN, model = "twoway", unit = "single", type = "agreement")
sad_occc_UN$occc
```
```{r}
sad_occc_UN$oprec
...
sad_icc_estimate_LIN <- icc(sad_estimate_LIN, model = "two", unit = "single", type = "agreement")
sad_occc_estimate_LIN$occc
```

```{r}
sad_occc_estimate_LIN$oprec
```

```{r}
sad_occc_estimate_LIN$oaccu
```
...
```{r}
sad_icc_estimate_LIN$ubound
```

```{r}
sad_icc_estimate_LIN$lbound
```
...
table2
write.csv(table2,"interrater reliability.csv")
```

```{r}

dat <- dat_cls %>%
  dplyr::select(rater,leaf, AID) 
model=lmer(AID~rater+(1|leaf),dat)
r2(model,ci=0.95)
...
```{r}
means=read.csv("E:/individual_leaf/means_no experience.csv",header=TRUE)
```
...
```{r}
cb=DescTools::MeanDiffCI(means$lshift2,means$lshift2,R=2000)
cb
sd(cb)
sd(means$Cb2)
sd(means$Cb1)
...
```{r}
lshift=DescTools::MeanDiffCI(means$lshift2,means$lshift1,R=2000)
lshift
sd(lshift)
sd(means$lshift2)
```
...
```{r}
sshift=DescTools::MeanDiffCI(means$s.shift2,means$s.shift1,R=2000)
sshift
sd(sshift)
```
...
m3=ggplot(data = dat_sad_estimate_LIN, aes(x = actual, y = estimate)) +
  ...
  scale_x_continuous(limits = c(0,100), name = "Actual severity (%)") +
  scale_y_continuous(limits = c(0,100), name = "Estimated severity (%)") +
  geom_text(data = rval.lab01, x = 0.9, y = 2.95, label = rval.lab01$lab) + 
  coord_fixed(ratio = 1/1)+
  theme(axis.text.x = element_text(angle = 90,size = 15))+
  theme(axis.text.y = element_text(angle = 0,size = 15))+
  theme(axis.title.x = element_text(size = 17))+
...
```{r}
m4=cowplot::plot_grid(m3,               m2+theme(axis.text.y = element_blank(),                          axis.ticks.y = element_blank(),                        axis.title.y = element_blank()),                         nrow=1,                  labels="auto",
                   align = "hv")
ggsave("m4.JPG",  height=8,width=7,dpi = 300)
...
```{r}
a=epi.ccc(actuals$actual,actuals$UN,
...
```{r}
a2 <- lm(actuals$actual ~ actuals$UN)
...
```{r}
library(ggplot2)
r1=ggplot(data = actuals, aes(x = actuals$actual, y = actuals$UN)) +
  theme_bw() +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) +
...
```{r}
library(ggplot2)
r2=ggplot(data = actuals, aes(x = actuals$actual, y = actuals$UN)) +
  theme_bw() +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) +
  geom_abline(data = a3, aes(intercept = alpha, slope = beta), 
...
```{r}
library(ggplot2)
r3=ggplot(data = actuals, aes(x = actuals$actual, y = actuals$UN)) +
  theme_bw() +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) +
  geom_abline(data = a3, aes(intercept = alpha, slope = beta), 
...
```{r}
library(ggplot2)
r4=ggplot(data = actuals, aes(x = actuals$actual, y = actuals$UN)) +
  theme_bw() +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) +
  geom_abline(data = a3, aes(intercept = alpha, slope = beta), 
...
```{r}
library(ggplot2)
r5=ggplot(data = actuals, aes(x = actuals$actual, y = actuals$UN)) +
  theme_bw() +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) +
  geom_abline(data = a3, aes(intercept = alpha, slope = beta), 
...
```{r}
library(ggplot2)
r6=ggplot(data = actuals, aes(x = actuals$actual, y = actuals$UN)) +
  theme_bw() +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) +
  geom_abline(data = a3, aes(intercept = alpha, slope = beta), 
...
```{r}
library(ggplot2)
r7=ggplot(data = actuals, aes(x = actuals$actual, y = actuals$UN)) +
  theme_bw() +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) +
  geom_abline(data = a3, aes(intercept = alpha, slope = beta), 
...
```{r}
library(ggplot2)
r8=ggplot(data = actuals, aes(x = actuals$actual, y = actuals$UN)) +
  theme_bw() +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) +
  geom_abline(data = a3, aes(intercept = alpha, slope = beta), 
...
```{r}
library(ggplot2)
r9=ggplot(data = actuals, aes(x = actuals$actual, y = actuals$UN)) +
  theme_bw() +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) +
  geom_abline(data = a3, aes(intercept = alpha, slope = beta), 
...
```{r}
library(ggplot2)
r10=ggplot(data = actuals, aes(x = actuals$actual, y = actuals$UN)) +
  theme_bw() +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) +
  geom_abline(data = a3, aes(intercept = alpha, slope = beta), 
...
```{r}
library(ggplot2)
r11=ggplot(data = actuals, aes(x = actuals$actual, y = actuals$UN)) +
  theme_bw() +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) +
  geom_abline(data = a3, aes(intercept = alpha, slope = beta), 
...
```{r}
library(ggplot2)
r12=ggplot(data = actuals, aes(x = actuals$actual, y = actuals$UN)) +
  theme_bw() +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) +
  geom_abline(data = a3, aes(intercept = alpha, slope = beta), 
...
```{r}
a=epi.ccc(actuals$actual,actuals$AID,
          ci = "z-transform", conf.level = 0.95, 
          rep.measure = FALSE)
a$rho.c
a$s.shift
...
```{r}
a2 <- lm(actuals$actual ~ actuals$AID)
alpha <- summary(a2)$coefficients[1,1]
beta <-  summary(a2)$coefficients[2,1]
a3 <- data.frame(alpha, beta)
```
...
```{r}
library(ggplot2)
b1=ggplot(data = actuals, aes(x = actuals$actual, y = actuals$UN)) +
  theme_bw() +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) +
  geom_abline(data = a3, aes(intercept = alpha, slope = beta), 
...
```{r}
library(ggplot2)
b2=ggplot(data = actuals, aes(x = actuals$actual, y = actuals$AID)) +
  theme_bw() +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) +
  geom_abline(data = a3, aes(intercept = alpha, slope = beta), 
...
```{r}
library(ggplot2)
b3=ggplot(data = actuals, aes(x = actuals$actual, y = actuals$AID)) +
  theme_bw() +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) +
  geom_abline(data = a3, aes(intercept = alpha, slope = beta), 
...
```{r}
library(ggplot2)
b4=ggplot(data = actuals, aes(x = actuals$actual, y = actuals$AID)) +
  theme_bw() +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) +
  geom_abline(data = a3, aes(intercept = alpha, slope = beta), 
...
```{r}
library(ggplot2)
b5=ggplot(data = actuals, aes(x = actuals$actual, y = actuals$AID)) +
  theme_bw() +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) +
  geom_abline(data = a3, aes(intercept = alpha, slope = beta), 
...
```{r}
library(ggplot2)
b6=ggplot(data = actuals, aes(x = actuals$actual, y = actuals$AID)) +
  theme_bw() +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) +
  geom_abline(data = a3, aes(intercept = alpha, slope = beta), 
...
```{r}
library(ggplot2)
b7=ggplot(data = actuals, aes(x = actuals$actual, y = actuals$AID)) +
  theme_bw() +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) +
  geom_abline(data = a3, aes(intercept = alpha, slope = beta), 
...
```{r}
library(ggplot2)
b8=ggplot(data = actuals, aes(x = actuals$actual, y = actuals$AID)) +
  theme_bw() +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) +
  geom_abline(data = a3, aes(intercept = alpha, slope = beta), 
...
```{r}
library(ggplot2)
b9=ggplot(data = actuals, aes(x = actuals$actual, y = actuals$AID)) +
  theme_bw() +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) +
  geom_abline(data = a3, aes(intercept = alpha, slope = beta), 
...
```{r}
library(ggplot2)
b10=ggplot(data = actuals, aes(x = actuals$actual, y = actuals$AID)) +
  theme_bw() +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) +
  geom_abline(data = a3, aes(intercept = alpha, slope = beta), 
...
```{r}
library(ggplot2)
b11=ggplot(data = actuals, aes(x = actuals$actual, y = actuals$AID)) +
  theme_bw() +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) +
  geom_abline(data = a3, aes(intercept = alpha, slope = beta), 
...
```{r}
library(ggplot2)
b12=ggplot(data = actuals, aes(x = actuals$actual, y = actuals$AID)) +
  theme_bw() +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) +
  geom_abline(data = a3, aes(intercept = alpha, slope = beta), 
